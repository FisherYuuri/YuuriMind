...
class YuuriMindConfig(PretrainedConfig):
    model_type = "yuurimind"
    def __init__(
        self,
        dropout: float = 0.0,
        bos_token_id: int = 1,
...
class Attention(nn.Module):
    def __init__(self, args: YuuriMindConfig):
        super().__init__()
...
class FeedForward(nn.Module):
    def __init__(self, config: YuuriMindConfig):
        super().__init__()
...
class YuuriMindBlock(nn.Module):
    def __init__(self, layer_id: int, config: YuuriMindConfig):
        super().__init__()
...
class YuuriMindModel(nn.Module):
    def __init__(self, config: YuuriMindConfig):
        super().__init__()
        self.config = config
        ...
        self.dropout = nn.Dropout(config.dropout)
        self.layers = nn.ModuleList([YuuriMindBlock(l, config) for l in range(self.num_hidden_layers)])
...
class YuuriMindForCausalLM(PreTrainedModel, GenerationMixin):
    config_class = YuuriMindConfig

    def __init__(self, config: YuuriMindConfig = None):
        self.config = config or YuuriMindConfig()
        super().__init__(self.config)
        self.model = YuuriMindModel(self.config)
